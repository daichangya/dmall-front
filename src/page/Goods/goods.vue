<template>
  <div class="goods">
    <div class="nav">
      <div class="w"  v-for="(item,i) in allCats" :key="i">
        <!--<a href="javascript:;" :class="{active:sortType===1}" @click="reset()">综合排序</a>-->
        <a href="javascript:;" @click="_getAllGoodsByCid(item.id)" :class="{active:sortType===2}">{{item.name}}</a>
...
<script>
  import { getAllGoods, getAllCatList } from '/api/goods.js'
  ...
  export default {
    ...
    methods: {
      ...
      _getAllCat () {
        getAllCatList().then(res => {
          if (res.success === true) {
            this.allCats = res.result
            this.error = false
          } else {
            this.error = true
          }
